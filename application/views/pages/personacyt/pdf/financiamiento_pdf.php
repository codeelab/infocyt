...
ob_start();
        ...
        {  
                ...
                { 
                        ...
                        $numero = $v->id_financiamiento;
                        $folio = $v->id_usuario;

                        $apoyo = $v->nombre_apoyo;
                        $programa = $v->nombre_programa;
                        $fecha = $v->fecha_inicio;
                        $final = $v->fecha_final;
                        $clabe01 = $v->palabra_clave_1;
                        $clabe02 = $v->palabra_clave_2;
                        $clabe03 = $v->palabra_clave_3;
                }
        }

            $transf = strtotime($fecha);
            $transf1 = strtotime($final);
        ...
        $w = array(80, 130);
        $al = array('L', 'L');
        $titulos = array("TIPO DE APOYO","TIPO DE PROGRAMA","FECHA INICIO","FECHA FINAL","PALABRA CLABE 01","PALABRA CLABE 02","PALABRA CLABE 03");
        $datos = array($apoyo,$programa,$fecha,$final,$clabe01,$clabe02,$clabe03);
        $fill = 0;
        // create new PDF document
        $pdf = new TCPDF('P', 'mm', 'A4', true, 'UTF-8', false);
        $pdf->SetTitle($folio.'-Financiamiento-'.$numero.'.pdf');
        ...
        $pdf->Cell(0, 7, "FICHA DE FINANCIAMIENTO", 'B', 0, 'R', 1);
        $pdf->Ln();
        ...
        $pdf->Output($folio.'-Financiamiento-'.$numero.'.pdf');
        unset($pdf)
