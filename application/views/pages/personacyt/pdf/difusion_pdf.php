...
ob_start();
        ...
        {  
                ...
                { 
                        ...
                        $numero = $v->id_difusion;
                        $folio = $v->id_usuario;

                        $titulo = $v->titulo_difusion;
                        $inicio = $v->fecha_inicio;
                        $tipo_part = $v->nombre_participacion;
                        $cyt_dependencia = $v->dependencia;
                        $cyt_financiamiento = $v->facilitadora;
                        $cyt_colaboracion = $v->colaboradores;
                        $qpcyt_dirigido = $v->nombre_dirigido;
                        $cyt_beneficiario = $v->beneficiario;
        ...
        $w = array(80, 130);
        $al = array('L', 'L');
        $titulos = array("TITULO", "FECHA DE INICIO","TIPO DE PARTICIPACIÓN","INSTITUCIÓN ORGANIZADORA","FINANCIAMIENTO","COLABORADORES","DIRIGIDO A", "BENEFICIARIOS","ES EN EL EXTRANJERO","PALABRA CLAVE 01", "PALABRA CLAVE 02","PALABRA CLAVE 03", "DESCRIPCIÓN");
        $datos = array($titulo, $fecha,$tipo_part,$cyt_dependencia,$cyt_financiamiento,$cyt_colaboracion,$qpcyt_dirigido,$cyt_beneficiario,$extranjero,$cyt_pal_clave01,$cyt_pal_clave02,$cyt_pal_clave03,$descripcion_larga);
        $fill = 0;
        // create new PDF document
        $pdf = new TCPDF('P', 'mm', 'A4', true, 'UTF-8', false);
        $pdf->SetTitle($folio.'-Difusion_Divulgacion-'.$numero.'.pdf');
        ...
        $pdf->Cell(0, 7, "FICHA DE DIFUSIÓN Y DIVULGACIÓN", 'B', 0, 'R', 1);
        $pdf->Ln();
        ...
        $pdf->Output($folio.'-Difusion_Divulgacion-'.$numero.'.pdf');
        unset($pdf)
