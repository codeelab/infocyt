...
ob_start();
        ...
        {  
                ...
                { 
                        ...
                        $nombre = $v->nombre_grupo;
                        $lider = $v->es_lider;
                        $descr = $v->descr_grupo ;
                        $nombrel = $v->nombre_lider;
                        $sector = $v->nombre_dirigido;
                        $institucion = $v->institucion_resp;
                        $dependencia = $v->descr_dependencia;
                        $departamento = $v->descr_departamentos;
                        $logros = $v->logros;
                        $actividades = $v->actividades;
                        $cyt_colaboracion = $v->colaboracion;
        ...
        $w = array(80, 130);
        $al = array('L', 'L');
        $titulos = array("NOMBRE","ES LIDER","NOMBRE DEL LIDER","SECTOR DE LA ENTIDAD ASESORA","INSTITUCIÓN EMPLEADORA","DEPENDENCIA DE LA INSTITUCIÓN","DEPARTAMENTO", "DESCRIPCIÓN","LOGROS","ACTIVIDADES","COLABORACIÓN","COMENTARIOS");
        $datos = array($nombre, $lider, $nombrel, $sector, $institucion, $dependencia, $departamento, $descr, $logros, $actividades, $cyt_colaboracion,$comentarios);
        $fill = 0;
        // create new PDF document
        $pdf = new TCPDF('P', 'mm', 'A4', true, 'UTF-8', false);
        $pdf->SetTitle($folio.'-Grupos-'.$numero.'.pdf');
        ...
        $pdf->Cell(0, 7, "FICHA DE GRUPO", 'B', 0, 'R', 1);
        $pdf->Ln();
        ...
        $pdf->Output($folio.'-Grupos-'.$numero.'.pdf');
        unset($pdf)
