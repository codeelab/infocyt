...
ob_start();
        ...
        {  
                ...
                { 
                        ...
                        $titulo = $v->titulo;
                        $fecha_publicacion = $v->fecha_pub;
                        $t_autor = $v->nombre_tipo;
                        $autor = $v->autor;
                        $total_autor = $v->total_autores;
                        $pais = $v->nombre_pa;
                        $tipo_p = $v->nombre_publicacion;
                        $comentarios = $v->comentarios;
                        $paginai = $v->pag_inicio;
                        $paginaf = $v->pag_final;
                        ...
                        $descr_larga = $v->descripcion_larga;

                }
        }

            $transf = strtotime($fecha_publicacion);
        ...
        $w = array(80, 130);
        $al = array('L', 'L');
        $titulos = array("TITULO","FECHA DE PUBLICACIÓN","TIPO DE AUTOR","NOMBRE DEL AUTOR","TOTAL DE AUTORES","PAÍS","TIPO DE PUBLICACIÓN","COMENTARIOS","PÁGINA INICIAL","PÁGINA FINAL","CAMPO DE CONOCIMIENTO","DISCIPLINA DE CONOCIMIENTO","SUBDISCIPLINA","DESCRIPCIÓN LARGA");
        $datos = array($titulo,$fecha,$t_autor,$autor,$total_autor,$pais,$tipo_p,$comentarios,$paginai,$paginaf,$ps_cyt_campo_id,$ps_cyt_disciplina_id,$ps_cyt_subdisciplina_id,$descr_larga);
        $fill = 0;
        // create new PDF document
        $pdf = new TCPDF('P', 'mm', 'A4', true, 'UTF-8', false);
        $pdf->SetTitle($folio.'-Reporte_Tecnico-'.$numero.'.pdf');
        ...
        $pdf->Cell(0, 7, "FICHA DE RESEÑA", 'B', 0, 'R', 1);
        $pdf->Ln();
        ...
        $pdf->Output($folio.'-Reporte_Tecnico-'.$numero.'.pdf');
        unset($pdf)
